...
export const updateJob = async (values) => {
	...
	console.log('values as at updateJob:>> ', values);

	const job = await readJobByJobCode(values.job_code);
	...
	const data = await axios.put(
		`${API_URL}/jobs/${job.id}`,
		{
			...
			job_type_visible: values.job_type_visible,
			job_description: values.job_desc,
			job_description_visible: values.job_desc_visible,
		},
		{
			headers: {
				'Content-Type': 'application/json',
